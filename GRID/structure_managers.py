...
class SingleDepot:
    ...
    def build_model(self, managers, objective_manager, expr_manager=None):
        ...
        self.location = self.converter.model.add_element_var(
            object_type=self.customer, target=self.converter.node_data["depots"][-1]
        )
        ...
        match self.vehicles_info:
            case "multi_types":
                self.vehicle_type = self.converter.model.add_object_type(
                    number=len(self.converter.vehicle_data["vehicles"])
                )
                self.vehicles = self.converter.model.add_element_var(
                    object_type=self.vehicle_type, target=0
                )
                self.vehicles_index = self.converter.model.add_element_table(
                    self.converter.vehicle_data["vehicles"]
                )
                self.vehicle_type_skip = self.converter.model.add_element_table(
                    self.converter.vehicle_data["skip"]
                )
            case "tsp":
                pass
            case "single_type":
                self.vehicles = self.converter.model.add_int_resource_var(
        ...
        self.converter.model.add_base_case(
            [self.unvisited.is_empty(), self.location == self.converter.node_data["depots"][-1]]
        )
    ...
    def _define_skip_transition(self):
        skip = dp.Transition(
            ...
            effects=[(self.vehicles, self.vehicle_type_skip[self.vehicles_index[self.vehicles]])],
            preconditions=[
        ...
        if expr_manager is None or not expr_manager.has_edges:
            for j in self.converter.node_data["customers"]:
                ...
                if self.objective_manager != expr_manager:
                    self.objective_manager.define_transition_cost(
                        self.location, j, "visit", visit, True
                    )
            ...
            for j in self.converter.node_data["customers"]:
                for index, (update, constraint) in enumerate(
                    safe_product(updates[j], constraints[j])
                ):
                    visit = dp.Transition(
                    ...
                    for manager in managers:
                        manager.manage_transition(visit, self.location, j, "visit")
                    expr_manager.manage_transition(
                        visit, self.location, j, "visit", update, constraint
                    )
                    if self.objective_manager != expr_manager:
                        self.objective_manager.define_transition_cost(
                            self.location, j, "visit", visit, True
    ...
    def _define_change_vehicle_transition(self, managers, expr_manager):
        if expr_manager is not None:
            if expr_manager.has_edges:
                ...
                for index, (update, constraint) in enumerate(
                    safe_product(
                        updates[self.converter.node_data["depots"][-1]],
                        constraints[self.converter.node_data["depots"][-1]],
                    )
                ):
                    ...
                    for manager in managers:
                        manager.manage_transition(
                            change_vehicle,
                            self.location,
                            self.converter.node_data["depots"][-1],
                            "change_vehicle",
                        )
                    expr_manager.manage_transition(
                        change_vehicle,
        ...
        if expr_manager.has_edges:
            ...
            for index, (update, constraint) in enumerate(
                ...
                for obj_index, obj_var in enumerate(obj_vars):
                    ...
                    expr_manager.define_transition_cost(
                        self.location,
                        self.converter.node_data["depots"][-1],
                        "change_vehicle",
                        change_vehicle,
    ...
    def _define_return_transition(self, managers, expr_manager):
        if expr_manager is None or not expr_manager.has_edges:
            return_to_depot = dp.Transition(
                name="return",
                cost=self.placeholder,
                effects=[(self.location, self.converter.node_data["depots"][-1])],
                preconditions=[
                    self.unvisited.is_empty(),
                    self.location != self.converter.node_data["depots"][-1],
                ],
            ...
            for manager in managers:
                manager.manage_transition(
                    return_to_depot, self.location, self.converter.node_data["depots"][-1], "return"
                )
            if expr_manager is not None:
                expr_manager.manage_transition(
                    return_to_depot, self.location, self.converter.node_data["depots"][-1], "return"
...
class MultiDepot:
    ...
    def _add_dummy_depot(self):
        dummy_depot = self.converter.routing_model.add_node(
            id=len(self.converter.routing_model.get_nodes()), depot=True
        )
    ...
    def build_model(self, managers, objective_manager, expr_manager=None):
        ...
        match self.vehicles_info:
            case "multi_types":
                ...
                self.allowed_starts = self.converter.model.add_set_table(
                    self.converter.vehicle_data["allowed_starts"], object_type=self.customer
                )
                self.allowed_ends = self.converter.model.add_set_table(
                    self.converter.vehicle_data["allowed_ends"], object_type=self.customer
                )
                self.vehicle_type_skip = self.converter.model.add_element_table(
                    self.converter.vehicle_data["skip"]
                )
    ...
    def _define_start_transition(self, managers, expr_manager):
        for depot in self.converter.node_data["depots"][:-1]:
            if self.vehicle_type == "multi_types":
                allowed_start = self.allowed_starts[self.vehicles_index[self.vehicles]].contains(
                    depot
                )
                start_path = dp.Transition(
                    name=f"start at depot {depot}",
                    cost=self.placeholder,
                    effects=[(self.location, depot)],
                    preconditions=[
                        self.location == self.converter.node_data["depots"][-1],
                        allowed_start,
                    ],
                ...
                if depot in self.allowed_starts:
                    start_path = dp.Transition(
                        ...
                        cost=self.placeholder,
                        effects=[(self.location, depot)],
                        preconditions=[self.location == self.converter.node_data["depots"][-1]],
                    )
                else:
                    start_path = None
            if start_path is not None:
                ...
                self.objective_manager.define_transition_cost(
                    self.location, depot, "start_path", start_path, False
                )
        ...
        if expr_manager is None or not expr_manager.has_edges:
            for j in self.converter.node_data["customers"]:
                ...
                if expr_manager is not None:
                    expr_manager.manage_transition(visit, self.location, j, "visit")
                self.objective_manager.define_transition_cost(
                    self.location, j, "visit", visit, False
                )
                self.converter.model.add_transition(visit)
        else:
            updates = expr_manager.edge_updates
    ...
    def _define_change_vehicle_transition(self, managers, expr_manager):
        if expr_manager is None or not expr_manager.has_edges:
            for depot in self.converter.node_data["depots"][:-1]:
                if self.vehicle_type == "multi_types":
                    allowed_end = self.allowed_ends[self.vehicles_index[self.vehicles]].contains(
                        depot
                    )
                    change_vehicle = dp.Transition(
                ...
                if change_vehicle is not None:
                    for manager in managers:
                        manager.manage_transition(
                            change_vehicle, self.location, depot, "change_vehicle"
                        )
                    if expr_manager is not None:
                        expr_manager.manage_transition(
                            change_vehicle, self.location, depot, "change_vehicle"
                    ...
                    self.objective_manager.define_transition_cost(
                        self.location, depot, "change_vehicle", change_vehicle, False
                    )
                    self.converter.model.add_transition(change_vehicle)
        else:
            updates = expr_manager.edge_updates
            ...
            for depot in self.converter.node_data["depots"][:-1]:
                if self.vehicle_type == "multi_types":
                    ...
                    for index, (update, constraint) in enumerate(
                        safe_product(updates[depot], constraints[depot])
                    ):
                        change_vehicle = dp.Transition(
                    ...
                    if depot in self.allowed_ends:
                        for index, (update, constraint) in enumerate(
                            ...
                            change_vehicle = dp.Transition(
                                ...
                                preconditions=[
                                    self.vehicles
                                    < len(self.converter.vehicle_data["vehicles"]) - 1,
                                    ~self.unvisited.is_empty(),
                                ]
                                + [self.location != d for d in self.converter.node_data["depots"]],
                            )
            ...
            for depot in self.converter.node_data["depots"][:-1]:
                if self.vehicle_type == "multi_types":
                    ...
                    return_to_depot = dp.Transition(
                        ...
                        preconditions=[self.unvisited.is_empty(), allowed_end]
                        + [self.location != d for d in self.converter.node_data["depots"]],
                    )
                else:
                    if depot in self.allowed_ends:
                ...
                if return_to_depot is not None:
                    ...
                    if expr_manager is not None:
                        expr_manager.manage_transition(
                            return_to_depot, self.location, depot, "return"
                        )
                    self.objective_manager.define_transition_cost(
                        self.location, depot, "return", return_to_depot, False
                    )
                    self.converter.model.add_transition(return_to_depot)
        else:
            updates = expr_manager.edge_updates
            ...
            for depot in self.converter.node_data["depots"][:-1]:
                if self.vehicle_type == "multi_types":
                    ...
                    for index, (update, constraint) in enumerate(
                        ...
                        expr_manager.manage_transition(
                            return_to_depot, self.location, depot, "return", update, constraint
                        )
                        self.objective_manager.define_transition_cost(
                            self.location, depot, "return", return_to_depot, False
                        )
                    ...
                    if depot in self.allowed_ends:
                        for index, (update, constraint) in enumerate(
                            ...
                            return_to_depot = dp.Transition(
                                ...
                                preconditions=[self.unvisited.is_empty()]
                                + [self.location != d for d in self.converter.node_data["depots"]],
                            )
                            for manager in managers:
                                manager.manage_transition(
