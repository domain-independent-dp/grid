...
class DemandsManager:
    def __init__(
        ...
        self.demands, self.types, self.capacities, self.multiple = self._manage_demand(
            demands_dict, demands_types, capacities_dict, capacities_types
        )
        ...
        if self.multiple:
            ...
            for i, demand in enumerate(self.demands):
                if self.types[i]:
                    ...
                    self.loads.append(
                        self.converter.model.add_int_resource_var(target=0, less_is_better=True)
                    )
                else:
                    ...
                    if self.vehicles_info == "multi_types":
                        ...
                        self.vehicles_indexing = self.structure.vehicles_index
                    self.loads.append(
                        self.converter.model.add_float_resource_var(target=0, less_is_better=True)
                    )
            self.demands = tables
            if self.vehicles_info == "multi_types":
            ...
            if self.types:
                ...
                if self.vehicles_info == "multi_types":
                    ...
                    self.vehicles_indexing = self.structure.vehicles_index
                    self.load = self.converter.model.add_float_resource_var(
            ...
            case "visit":
                if self.multiple:
                    for i in range(self.number):
                        if self.vehicles_info != "multi_types":
                            transition.add_precondition(
                                self.loads[i] + self.demands[i][j] <= self.capacities[i]
                            )
                        else:
                            vehicle_index = self.vehicles_indexing[self.structure.vehicles]
                            transition.add_precondition(
                                self.loads[i] + self.demands[i][j]
                                <= self.capacities[i][vehicle_index]
                            )
                    ...
                    if self.vehicles_info != "multi_types":
                        ...
                        vehicle_index = self.vehicles_indexing[self.structure.vehicles]
                        transition.add_precondition(
                            self.load + self.demands[j] <= self.capacities[vehicle_index]
                        )
    ...
    def manage_state_constraints(self, must_visit, mandatory_nodes):
        if self.multiple:
            for i in range(self.number):
                ...
                if self.vehicles_info == "single_type":
                    ...
                    self.converter.model.add_state_constr(
                        (total_vehicles - self.structure.vehicles) * self.capacities[i]
                        - self.loads[i]
                        >= total_demand
                    )
                    ...
                    for v in range(len(self.converter.vehicle_data["vehicles"])):
                        remaining_capacity = remaining_capacity + (
                            v >= self.structure.vehicles
                        ).if_then_else(self.capacities[i][self.vehicles_indexing[v]], 0)
                    self.converter.model.add_state_constr(
                        remaining_capacity - self.loads[i] >= total_demand
                    )
                else:
                    self.converter.model.add_state_constr(
                        self.capacities[i] - self.loads[i] >= total_demand
                    )
        else:
            ...
            if self.vehicles_info == "single_type":
                total_vehicles = len(self.converter.vehicle_data["vehicles"])
                self.converter.model.add_state_constr(
                    (total_vehicles - self.structure.vehicles) * self.capacities - self.load
                    >= total_demand
                )
            elif self.vehicles_info == "multi_types":
                remaining_capacity = 0
                for v in range(len(self.converter.vehicle_data["vehicles"])):
                    remaining_capacity = remaining_capacity + (
                        v >= self.structure.vehicles
                    ).if_then_else(self.capacities[self.vehicles_indexing[v]], 0)
                self.converter.model.add_state_constr(
                    remaining_capacity - self.load >= total_demand
                )
            else:
...
class TimeWindowsManager:
    ...
    def expand_model(self, t):
        if t is not None:
            ...
            if self.times_type:
                ...
                self.time = self.converter.model.add_float_resource_var(
                    target=0, less_is_better=True
                )
        if self.times_type:
            self.time = self.converter.model.add_int_resource_var(target=0, less_is_better=True)
    ...
    def manage_state_constraints(self, must_visit, mandatory_nodes):
        if self.shortest_cost is not None:
            ...
            if (
                not self.triangular_inequality
                or self.converter.edge_data["connections"] is not None
            ):
                ...
                for k in range(n):
                    for i in range(n):
                        for j in range(n):
                            if (
                                shortest_distance_matrix[i][j]
                                > shortest_distance_matrix[i][k] + shortest_distance_matrix[k][j]
                            ):
                                shortest_distance_matrix[i][j] = (
                                    shortest_distance_matrix[i][k] + shortest_distance_matrix[k][j]
                                )
                if self.times_type:
                    ...
                    shortest_distance = self.converter.model.add_float_table(
        ...
        if self.vehicles_info == "tsp":
            for j in self.converter.node_data["customers"]:
                ...
                if end is not None:
                    self.converter.model.add_state_constr(
                        ~mandatory_nodes.contains(j)
                        | (self.time + shortest_distance[self.structure.location, j] <= end)
                    )
...
class PickUpAndDeliveryManager:
    def __init__(
        self, converter, commodities_dict, commodities_type, pd_capacities, pd_type, vehicles_info
    ):
        self.converter = converter
        self.structure = self.converter.structure
        ...
        self.vehicles_info = vehicles_info
        self.split, self.deltas, self.predecessors, self.successors = self._manage_commodities(
            commodities_dict
        )
    ...
    def expand_model(self, _):
        ...
        if self.common_var:
            if self.commodities_type:
                self.didp_vars["Common"] = self.converter.model.add_int_resource_var(
                    target=0, less_is_better=True
                )
            else:
                self.didp_vars["Common"] = self.converter.model.add_float_resource_var(
                    target=0, less_is_better=True
                )
        for key in self.split["M-M"]:
            if self.commodities_type:
                self.didp_vars[key] = self.converter.model.add_int_resource_var(
                    target=0, less_is_better=True
                )
            else:
                self.didp_vars[key] = self.converter.model.add_float_resource_var(
                    target=0, less_is_better=True
                )
        if self.vehicles_info == "multi_types":
            self.vehicles_indexing = self.structure.vehicles_index
        ...
        self.predecessors_table = self.converter.model.add_set_table(
            self.predecessors, object_type=self.structure.customer
        )
        if self.successors is not None:
            self.must_visit = self.converter.model.add_set_var(
                target=[], object_type=self.structure.customer
            )
            self.successors_table = self.converter.model.add_set_table(
                self.successors, object_type=self.structure.customer
            )
    ...
    def _manage_commodities(self, commodities_dict):
        ...
        successors = (
            [set() for _ in range(len(self.converter.nodes))]
            if self.vehicles_info != "tsp"
            else None
        )
                ...
                if self.common_var:
                    if self.deltas["Common"][j] != 0:
                        ...
                        transition.add_effect(
                            self.didp_vars["Common"],
                            self.didp_vars["Common"] + self.deltas["Common"][j],
                        )
                    if self.successors is not None:
                        if len(self.successors[j]) > 0:
                            transition.add_effect(
                                self.must_visit,
                                self.must_visit.union(self.successors_table[j]).remove(j),
                            )
                        else:
                            transition.add_effect(self.must_visit, self.must_visit.remove(j))
                    if len(self.predecessors[j]) > 0:
                        transition.add_precondition(
                            self.structure.unvisited.isdisjoint(self.predecessors_table[j])
                        )
                for key in self.split["M-M"]:
                    if self.deltas[key][j] != 0:
                        ...
                        transition.add_effect(
                            self.didp_vars[key], self.didp_vars[key] + self.deltas[key][j]
                        )
...
class VisitManager:
    ...
    def expand_model(self, _):
        ...
        if self.optional_nodes:
            self.optional_nodes_set = self.converter.model.create_set_const(
                object_type=self.structure.customer, value=self.optional_nodes
            )
        ...
        self.unvisited = self.converter.model.add_set_var(
            object_type=self.structure.customer,
            target=[
                c
                for c in customers
                if not (c in self.multiple_visits.keys() and c in self.optional_nodes)
            ],
        )
            ...
            case "visit":
                if j in self.multiple_visits:
                    ...
                    if var is not None:
                        ...
                        transition.add_effect(
                            self.multiple_visits_limits[j], self.multiple_visits_limits[j] - 1
                        )
                else:
