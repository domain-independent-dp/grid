...
from .features_managers import (
    ConnectionsManager,
    DemandsManager,
    PickUpAndDeliveryManager,
    TimeWindowsManager,
    VisitManager,
)
from .objective_managers import DistanceManager, TimeManager, VehiclesManager
from .structure_managers import MultiDepot, SingleDepot
...
class DIDPConverter:
    def __init__(self, routing_model, maximize=False):
        ...
        if len(self.model_vars) + len(self.vehicles_vars) > 0:
            self.expr_manager = ExpressionsManager(
                self,
                self.vehicles_vars,
                self.model_vars,
                self.routing_model.constraints,
                self.edge_data["updates"],
                self.edge_data["constraints"],
                self.node_data["updates"],
                self.node_data["constraints"],
                self.vehicles_info,
            )
    ...
    def _init_objective_manager(self):
        ...
        if isinstance(obj, Var):
            ...
            self.model = dp.Model(
                float_cost=not self.edge_data["distance"][1], maximize=self.maximize
            )
    ...
    def _init_managers(self):
        ...
        managers.append(
            VisitManager(
                self, self.node_data["optional customers"], self.node_data["multiple visits"]
            )
        )
        ...
        if self.node_data["demands"] is not None:
            managers.append(
                DemandsManager(
                    self,
                    self.node_data["demands"][0],
                    self.node_data["demands"][1],
                    self.vehicle_data["capacities"][0],
                    self.vehicle_data["capacities"][1],
                    self.vehicles_info,
                )
            )
        ...
        if self.node_data["time_intervals"] is not None:
            ...
            shortest_cost = (
                self.routing_model.shortest_time
                if self.routing_model.shortest_time is not None
                else None
            )
            ...
            if use_distance:
                ...
                shortest_cost = (
                    self.routing_model.shortest_distance
                    if self.routing_model.shortest_distance is not None
                    else None
                )
            ...
            managers.append(
                TimeWindowsManager(
                    ...
                    time_cost,
                    shortest_cost,
                    times_type and cost_type,
                    self.vehicles_info,
                    use_distance,
                    self.routing_model.triangular_inequality,
                )
            )
        ...
        if self.node_data["commodities"] is not None:
            managers.append(
                PickUpAndDeliveryManager(
                    self,
                    self.node_data["commodities"][0],
                    self.node_data["commodities"][0],
                    self.vehicle_data["pd_capacities"][0],
                    self.vehicle_data["pd_capacities"][1],
                    self.vehicles_info,
                )
            )
